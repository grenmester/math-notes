...
$\varphi:G\to G'$ is an isomorphism if and only if there exists
$\psi:G'\to G$ such that $\psi$ is a homomorphism and
...
\begin{bdefi}
    ...
    \begin{enumerate}
        ...
        \item The projection map $\pi:G\to G'=G$/$\sim$ is a group
          homomorphism.
    \end{enumerate}
\end{bdefi}

\begin{defi}
    Let $\varphi:G\to G'$ be a homomorphism and let $g'\in G'$. The \define{fiber}
...
\begin{bpf}
    Let $\varphi:G\to G'$ be a homomorphism, then $\varphi$ induces an equivalence
    ...
    $\pi:G\to G'$ is the projection. But $\pi(x)=\pi(y)\Leftrightarrow
    xy^{-1}\in\ker\varphi$.
\end{bpf}
...
\begin{prop}
    Let $\varphi:G\to G'$ be a homomorphism and $K=\ker\varphi$. Then $xKx^{-1}\subseteq
    K$, $\forall x\in G$.
\end{prop}
...
N$. Then, $G$/$\sim=G/N$, $[x]=xN$, $\pi:G\to G/N$, $\pi(x)=xN$,
$\ker\pi=N$.
...
\begin{pf}
    ...
    $L\rightarrow R$. Define $f:R\to L$ by $f(Hx)=x^{-1}H$, and define
    $g:L\to R$ by $g(xH)=Hx^{-1}$. Then $f$ and $g$ are mutually inverse.
...
Let $\varphi:G\to G'$ be a homomorphism. How far is $\varphi$ from an isomorphism?
...
\begin{bthm}[First Isomorphism Theorem]
    Let $\varphi:G\to G'$ be a homomorphism. Then $\ker\varphi\unlhd G$, $\im\varphi\le G'$ and
...
\begin{pf}
    Define $\theta:G/\ker\varphi\to \im\varphi$ by $\theta(x\ker\varphi)=\varphi(x)$.\\\\
    First we show that $\theta$ is well-defined. Suppose $x\ker\varphi=y\ker\varphi$. Then,
    \begin{align*}
        x\ker\varphi=y\ker\varphi&\Leftrightarrow y^{-1}x\ker\varphi=\ker\varphi\\
...
\begin{pf}
    Consider $\psi:G\to \Inn(G)$ defined by $x\mapsto T_x$. Then,
    $\psi$ is surjective, i.e. $\im\psi=\Inn(G)$. We then determine
...
\begin{bpf}
    ...
    $\omega:G/A\to G/B$ such that $\omega(xA)=xB$. We show that $\omega$
...
\begin{prop}
    There is an isomorphism $\theta:(G/A)/(B/A)\to G/B$ such that this
    diagram commutes.
    \begin{center}
...
\begin{bpf}
    Let $\varphi:A\to AN/N$ such that $a\mapsto aN$. Then by the first isomorphism
...
\begin{thm}[First Isomorphism Theorem for Rings]
    If $\varphi:R\to S$ is homomorphism of rings, then $\ker\varphi$ is an ideal of $R$,
    $\im\varphi$ is a subring of $S$, and
