...
\begin{prop}
    ...
    \begin{center}
        \begin{tikzcd}[column sep=huge, row sep=huge, every matrix/.append style={name=m}, execute at end picture={
                    \draw [<-] ([xshift=-4.6em, yshift=1.2em]m-2-2.north) arc[start angle=-90,delta angle=270,radius=0.25cm];
            }]
...
\begin{bpf}
    ...
    Now we prove $(G/A)/(B/A)\cong(G/B)$. We define the homomorphism $\hmap[\omega]{G/A}{B/A}$ such that $\omega(xA)=xB$. We show that $\omega$ is well-defined. If $xA=yA$, then
    \begin{align*}
